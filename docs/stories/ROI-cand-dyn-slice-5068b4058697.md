# Story: [1. Como usar este documento] Cada item só é marcado como concluído quando existir evidênc · Código existente sem execução comprovada não é considerado c · Teste unitário isolado não substitui execuçã

**Story ID:** `ROI-cand-dyn-slice-5068b4058697`  
**Epic:** EPIC-EXTRA-DOD-ROI (evergreen)  
**Status:** Done  
**Risk level:** **HIGH-RISK**  
**Source:** squad `extra-dod-roi` force-next (cycle `cyc-2026-07-18T134622Z`)  
**Candidate ID:** `cand-dyn-slice:5068b4058697`  
**ROI:** `2.4928`  
**DoD refs:** 1. Como usar este documento, Cada item só é marcado como concluído quando existir evidência verificável., Código existente sem execução comprovada não é considerado concluído., Teste unitário isolado não substitui execução ponta a ponta., Alterações de escopo são refletidas primeiro neste documento e nos documentos ca, Itens explicitamente marcados como opcionais não bloqueiam o fechamento do proje
...
I want **[1. Como usar este documento] Cada item só é marcado como concluído quando existir evidênc · Código existente sem execução comprovada não é considerado c · Teste unitário isolado não substitui execuçã**,  
...
Open local-stage items in section '1. Como usar este documento' without VPS/live/human-accept blocker patterns
...
- Work defined by candidate `cand-dyn-slice:5068b4058697` only
...
| 2026-07-18 | @po | Ready |
| 2026-07-18 | @qa/@po | PASS process integrity |
