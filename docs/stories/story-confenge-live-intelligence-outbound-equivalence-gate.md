...
| ~~TD-LI-6~~ **(transferido — não é mais absorvido por esta story)** | Não-determinismo de `test_blocked_when_watermark_is_missing` por poluição do banco de teste compartilhado, aceito como dívida não-bloqueadora pelo @po em RULING-LI-03 (v1.8) | **Dono formal do fechamento: `docs/stories/story-confenge-live-intelligence-w2-web-export.md`, AC10** (banco isolado `extra_li_equiv` via `scripts/ops/li_equiv_db.py`/`make li-equiv`; reexecução de `test_blocked_when_watermark_is_missing` com asserções originais intactas e `DELETE ... LIKE 'LI-TEST-%'` inalterado). Reconciliação do @po em `*validate-story-draft` da W2 (2026-09-03): a W2 traz um critério de fechamento **concreto e testável**; a absorção anterior aqui era **especulativa** ("resolveria como efeito colateral"). Se a W2 for reescopada e perder o AC10, TD-LI-6 volta a ser dívida sem portador e deve ser reatribuído explicitamente — não retorna a esta story por default |
...
| 2026-09-03 | 1.2 | **Reconciliação de titularidade de TD-LI-6 (@po, durante `*validate-story-draft` da `story-confenge-live-intelligence-w2-web-export.md`).** TD-LI-6 deixa de ser absorvido por esta story e passa a ter dono formal em **W2/AC10** (`extra_li_equiv`, `make li-equiv`, reexecução de `test_blocked_when_watermark_is_missing` sem tocar asserções nem o escopo do `DELETE LI-TEST-`). Motivo: a W2 traz critério de fechamento concreto e testável; a absorção anterior era especulativa. **Adjudicação adicional:** `extra_li_equiv` **não** descarrega a dependência bloqueante "@devops provisiona 2º DSN" desta story — `extra_li_equiv` é **um** banco em nível de migration ≤105, enquanto o protocolo de dois braços exige **dois** bancos em níveis **distintos** (Banco A ≤102, Banco B ≤104+). A dependência de @devops permanece **intacta e bloqueante**. Alterada **apenas** a linha TD-LI-6 da tabela de débitos — nenhum AC, escopo, `closure-key` ou linha de TD-LI-1/AR-3/AR-4/REL-004/REL-005 foi tocado. Status permanece **Draft**. | @po (Pax) |
