# Story: [7.1 Registry de fontes] Cada fonte informa estratégia de retry. · Cada fonte informa estratégia de backoff. · Cada fonte informa status operacional. (+5 more)

**Story ID:** `ROI-cand-dyn-slice-d8deaa518e86`  
**Epic:** EPIC-EXTRA-DOD-ROI (evergreen)  
**Status:** Done  
**Risk level:** **HIGH-RISK**  
**Source:** squad `extra-dod-roi` force-next (cycle `cyc-2026-07-18T160023Z`)  
**Candidate ID:** `cand-dyn-slice:d8deaa518e86`  
**ROI:** `1.8067`  
**DoD refs:** 7.1 Registry de fontes, Cada fonte informa estratégia de retry., Cada fonte informa estratégia de backoff., Cada fonte informa status operacional., Cada fonte informa data da última validação., Cada fonte informa bloqueadores conhecidos.
...
I want **[7.1 Registry de fontes] Cada fonte informa estratégia de retry. · Cada fonte informa estratégia de backoff. · Cada fonte informa status operacional. (+5 more)**,  
...
- Work defined by candidate `cand-dyn-slice:d8deaa518e86` only
...
| **Date** | 2026-07-18 |
| **Verdict** | **CONCERNS** |
| **Evidence** | `docs/ops/session-2026-07-18-source-registry-7-1b/QA-VERDICT.md` |
| **Tests** | `tests/test_source_registry_dod71.py` — 8 passed |
| **Validate** | `ok=true` n_sources=11 active=3 implemented_not_proven=8 |

### Summary

Remainder DoD §7.1 items (retry, backoff, operational status, last validation, blockers, role, status machine, crawler≠active) are **substantively DONE** with live export + unit status machine. Residual: AC3 process order (DoD/`state` pre-stamped before independent verdict), `active` allowed without `last_validation_at`, blocked/not_applicable synthetic-only.

### Gate implication

Story may remain Done under CONCERNS. Full write-up: session pack `QA-VERDICT.md`.
...
| 2026-07-18 | @qa (Quinn) | Independent review → **CONCERNS**; QA Results + session QA-VERDICT.md |
