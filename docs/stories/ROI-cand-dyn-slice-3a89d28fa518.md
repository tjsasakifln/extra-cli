# Story: [23. Observabilidade e alertas] Freshness por fonte é monitorada. · Coverage por capability é monitorada. · Último backup válido é monitorado. (+5 more)

**Story ID:** `ROI-cand-dyn-slice-3a89d28fa518`  
**Epic:** EPIC-EXTRA-DOD-ROI (evergreen)  
**Status:** Done  
**Risk level:** **HIGH-RISK**  
**Source:** squad `extra-dod-roi` force-next (cycle `cyc-2026-07-18T143244Z`)  
**Candidate ID:** `cand-dyn-slice:3a89d28fa518`  
**ROI:** `2.0962`  
**DoD refs:** 23. Observabilidade e alertas, Freshness por fonte é monitorada., Coverage por capability é monitorada., Último backup válido é monitorado., Falhas de migration são monitoradas., Timers atrasados são monitorados.
...
I want **[23. Observabilidade e alertas] Freshness por fonte é monitorada. · Coverage por capability é monitorada. · Último backup válido é monitorado. (+5 more)**,  
...
- cand-dyn-slice:fb2b26f3e596 ROI=2.0962 preterido vs #1: menor combinação gate+risco+evidência ou maior custo/dependência externa
...
- Work defined by candidate `cand-dyn-slice:3a89d28fa518` only
...
**Cycle:** `cyc-2026-07-18T143244Z`  
**Reviewed commit:** `96fa896`  
**Artifact:** `squads/extra-dod-roi/state/qa/cyc-2026-07-18T143244Z-qa.json`  
**Independence:** yes (implementer = delivery-engineer)

### Evidence re-run
| Command | Result |
|---------|--------|
| `pytest tests/test_ops_gate_monitor.py -q --no-cov` | 5 passed |
| `python3 -m scripts.ops.ops_gate_monitor --json` | overall=degraded; 5 checks; all monitored=true |
| falsify timers (no systemctl) | status=**configured**, not ok from repo alone |
| falsify 95% claim | forbidden listed; not in allowed/message |
| `DOD.md` §23 lines 1333–1344 | all still `[ ]` (no pre-flip) |

### DoD items authorized to flip after this PASS (exactly **5**)
1. `dod:38bd94e15cf4` — Freshness por fonte é monitorada.
2. `dod:007fbf01bbfb` — Coverage por capability é monitorada.
3. `dod:919378af4818` — Último backup válido é monitorado.
4. `dod:fdb23a7c688a` — Falhas de migration são monitoradas.
5. `dod:170c4028041e` — Timers atrasados são monitorados.

### Must stay open
- `dod:20bead44fa35` — Alertas possuem destino configurado.
- `dod:6014e0f6596a` — O destino de alerta foi testado.
- `dod:2de9e2601135` — O alerta possui contexto suficiente para ação.
- Rest of alert stack (tempestade / rate limiting / webhook / fallback)

QA does **not** flip `DOD.md`. @po applies the 5 flips with evidence, then closes.

### Residual (non-blocking)
- Freshness often `unknown` until live success timestamps feed health.
- Timers delayed detection needs systemctl on host; local stage proves inventory + configured.
...
| 2026-07-18 | @po | Ready |
| 2026-07-18 | delivery-engineer | ops_gate_monitor (DoD flip after QA) |
| 2026-07-18 | @qa adversarial | PASS — 5 ops-gate flips; alert stack open |

---

*Generated by squads/extra-dod-roi/scripts/materialize_aiox_story.py — do not hand-edit candidate_id binding.*
| 2026-07-18 | @po | Closed QA PASS; flipped 5 ops-gate DoD items |
