# Story: [27. Organização e manutenção do código] Nomes de módulos são consistentes. · Imports funcionam sem hacks de `sys.path` desnecessários. · Funções públicas possuem docstring quando necessário. (+5 more

**Story ID:** `ROI-cand-dyn-slice-55dc8958c51c`  
**Epic:** EPIC-EXTRA-DOD-ROI (evergreen)  
**Status:** Done  
**QA independent verdict:** **CONCERNS** (see QA Results; residual debt documented, not FAIL)  
**Risk level:** **HIGH-RISK**  
**Source:** squad `extra-dod-roi` force-next (cycle `cyc-2026-07-18T162552Z`)  
**Candidate ID:** `cand-dyn-slice:55dc8958c51c`  
**ROI:** `1.6535`  
**DoD refs:** 27. Organização e manutenção do código, Nomes de módulos são consistentes., Imports funcionam sem hacks de `sys.path` desnecessários., Funções públicas possuem docstring quando necessário., Funções críticas possuem type hints., Exceções são específicas.
...
I want **[27. Organização e manutenção do código] Nomes de módulos são consistentes. · Imports funcionam sem hacks de `sys.path` desnecessários. · Funções públicas possuem docstring quando necessário. (+5 more**,  
...
- Work defined by candidate `cand-dyn-slice:55dc8958c51c` only
...
**Reviewer:** Quinn (@qa)  
**Date:** 2026-07-18  
**Scope:** DoD §27 first 5 only (module names · sys.path policy · docstrings · type hints · critical-path exceptions)  
**Verdict:** **CONCERNS**

### Evidence re-run

- `python3 -m pytest tests/test_code_organization_gate.py -q --no-cov -o addopts=` → **3 passed**
- `python3 -m scripts.ops.code_organization_gate --json` → `summary.ok=true`, `n_py_files=296`
- Full write-up: `docs/ops/session-2026-07-18-code-org/QA-VERDICT.md`

### Per-item (first 5)

| # | Item | Status |
|---|------|--------|
| 1 | Nomes de módulos consistentes | DONE (`n_non_snake=0`) |
| 2 | sys.path sem hacks desnecessários | PARTIAL — policy+inventory (86 inserts / 18 disallowed); **not** zero |
| 3 | Docstrings públicas | DONE (`docstring_pct=51.9`) |
| 4 | Type hints críticos | DONE (`return_hint_pct=100`) |
| 5 | Exceções específicas (critical path) | DONE (`n_critical_path=0`) |

### Falsification

- Claim “zero `sys.path` repo-wide” → **REJECTED** (`n_inserts=86`, forbidden claims list)
- Claim “zero `except Exception: pass` repo-wide” → correctly **not** claimed (25 remain; open DoD items)

### Residual (non-blocking)

1. 18 disallowed sys.path inserts (brownfield debt; soft threshold `<20`)
2. `summary.ok` does not include `sys_path.ok`
3. Thin tests (no assert on anti-zero claim / full summary)
4. Process: state had orchestrator `PASS`/`Done` before independent `QA-VERDICT.md` — this section is the independent record

**Gate:** CONCERNS — first 5 substantively evidenced with honest inventory; residuals documented. Not FAIL.
...
| 2026-07-18 | Quinn (@qa) | Independent QA on §27 first 5 → **CONCERNS**; wrote session QA-VERDICT.md |
