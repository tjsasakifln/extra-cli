...
def collect(row):
    out = dict(row)
    try: cnpj, year, seq = parse_id(row["contrato_id"])
    except Exception as exc:
        out["fatal_error"] = repr(exc); return out
    ...
    if purchase_id:
        pm = re.fullmatch(r"([A-Za-z0-9]{14})-1-(\d+)/([0-9]{4})", str(purchase_id))
        if pm:
            pc, py, ps = pm.group(1), int(pm.group(3)), int(pm.group(2))
            proot = f"{BASE}/{pc}/compras/{py}/{ps}"
            procurement = get_json(proot)
            procurement_files = get_json(proot + "/arquivos")
            procurement_history = get_json(proot + "/historico", params={"pagina": 1, "tamanhoPagina": 500})
    term_details, seen = [], set()
    for term in unwrap_list(terms.get("data")):
        s = term_seq(term)
        if s is None or s in seen: continue
        seen.add(s)
        term_details.append({"sequencial": s, "list_item": term, "detail": get_json(root + f"/termos/{s}"), "files": get_json(root + f"/termos/{s}/arquivos")})
    for event in unwrap_list(history.get("data")):
        if not isinstance(event, dict): continue
        try: s = int(event.get("sequencialTermoContrato"))
        except Exception: continue
        if s in seen: continue
        seen.add(s)
        term_details.append({"sequencial": s, "list_item": None, "detail": get_json(root + f"/termos/{s}"), "files": get_json(root + f"/termos/{s}/arquivos")})
    out.update({"contract_url": root, "contract": contract, "contract_files": contract_files, "terms": terms, "term_details": term_details, "history": history, "empenhos": empenhos, "procurement": procurement, "procurement_files": procurement_files, "procurement_history": procurement_history})
    return out
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for index, future in enumerate(concurrent.futures.as_completed(futures), 1):
            ...
            except Exception as exc: results.append({**row, "fatal_error": repr(exc)})
    ...
    summary = {"contracts": len(results), "contract_ok": sum(1 for row in results if row.get("contract", {}).get("http") == 200), "with_terms": sum(1 for row in results if row.get("term_details")), "with_procurement": sum(1 for row in results if row.get("procurement", {}).get("http") == 200), "fatal": sum(1 for row in results if row.get("fatal_error"))}
