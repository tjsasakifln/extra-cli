...
class MultiSourceBackfill:
    ...
    def _save_checkpoint(self):
        ...
        try:
            ...
            logger.info(
                "Checkpoint salvo: %d fontes concluidas, iteracao %d",
    ...
    def _load_checkpoint(self) -> bool:
        ...
        try:
            ...
            self.stats = data
            logger.info(
                "Checkpoint carregado: %d fontes concluidas, iteracao %d",
                len(self.stats.get("sources_done", [])),
                self.stats.get("iterations", 0),
            )
            return True
    ...
    def _generate_report(self):
        ...
        if skipped:
            ...
            for skip in skipped:
                lines.append(
                    f"| {skip.get('source', 'N/A')} | {skip.get('iteration', 'N/A')} | {skip.get('reason', 'N/A')} |"
                )
            lines.append("")
        ...
        if self.stats["entities_after"] < COVERAGE_TOTAL_ENTITIES:
            ...
            if pct_final < 85:
                lines.append(
                    "**Target primario NAO atingido.** O relatorio de gap deve ser usado como input para COVERAGE-3.4.\n"
                )
            elif pct_final >= 95:
        ...
        lines.append("---\n")
        lines.append(
            f"*Relatorio gerado automaticamente por COVERAGE-3.3 backfill pipeline em {self.stats.get('completed_at', 'N/A')}*\n"
        )
    ...
    def run_pipeline(
        ...
        while iteration < MAX_ITERATIONS:
            ...
            for source in sources:
                ...
                if status == "success":
                    ...
                    logger.info(
                        "Fonte %s SUCCESS. fetched=%d inserted=%d matched=%d new_covered=%d",
                        source,
            ...
            if new_entities_this_iter == 0:
                ...
                logger.info("Iteracao %d: +%d novas entidades nesta iteracao", iteration, new_entities_this_iter)
        ...
        if not dry_run:
            ...
            simulated_total = len(self.stats.get("sources_done", [])) * (
                simulate_matches if simulate_matches > 0 else 0
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("=" * 60)
    print(
        f"  Cobertura: {stats['entities_before']} -> {stats['entities_after']} "
