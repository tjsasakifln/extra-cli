...
def build_pdf(data, output_path, pacote_recomendado="semanal"):
    ...
    story.append(Paragraph("Os dados são públicos. A análise é o diferencial.", S["BodyBold"]))
    story.append(PageBreak())
        ...
        make_table(
            ...
            [
                [
                    "1",
                    "Varredura",
                    "Monitoramento contínuo de 100% das publicações obrigatórias por lei (Lei 14.133/2021)",
                ],
                ["2", "Triagem Setorial", "Classificação automática por IA — zero ruído, só editais do seu setor"],
                [
                    "3",
                    "Análise Individual",
                    "Cada edital avaliado em 17 dimensões: objeto, valor, modalidade, "
                    "prazo, exigências de habilitação, atestados, índices contábeis, consórcio...",
                ],
                [
                    "4",
                    "Análise Documental",
                    "PDF do edital lido integralmente — checklist de 25 itens de "
                    "habilitação, identificação de cláusulas restritivas e red flags",
                ],
                [
                    "5",
                    "Score de Compatibilidade",
                    "Avaliação com 7 fatores calibrados ao perfil da empresa: "
                    "porte, capital, experiência, localização, modalidade, prazo, complexidade",
                ],
                [
                    "6",
                    "Recomendação Final",
                    "PARTICIPAR ou NÃO PARTICIPAR com justificativa detalhada — "
        ...
        make_table(
            ...
            [
                ...
                [
                    ...
                    "As melhores oportunidades entre centenas analisadas, com recomendação individual",
                ],
                [
                    "Análise de Habilitação",
                    "25 itens verificados por edital — antes de investir tempo na proposta, "
                    "você sabe se tem chance real",
                ],
                [
                    "Cláusulas Restritivas",
                    "Identificação de exigências que eliminam 60% das empresas antes do julgamento",
                ],
    ...
    story.append(hr())
    story.append(Paragraph("Três opções dimensionadas para diferentes níveis de acompanhamento.", S["Body"]))
    story.append(Spacer(1, 2 * mm))
    ...
    oferta_label = Table([[P("OFERTA POR TEMPO LIMITADO", "BadgeWhite")]], colWidths=[60 * mm])
    oferta_label.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, -1), NAVY),
                ("ALIGN", (0, 0), (-1, -1), "CENTER"),
    ...
    cond_inner = Table([[Paragraph(cond_text, S["Body"])]], colWidths=[CONTENT_W - 12 * mm])
    cond_inner.setStyle(
        TableStyle(
            [
    ...
    story.append(Paragraph("Estes serviços podem ser contratados separadamente sob demanda.", S["Small"]))
    story.append(PageBreak())
    ...
    story.append(
        Paragraph(
            "Disponível para uma conversa sem compromisso para esclarecer qualquer ponto desta proposta.", S["Body"]
        )
    )
    story.append(Spacer(1, 8 * mm))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Gerador de PDF de Proposta Comercial B2G v5")
    parser.add_argument(
        "input_positional", nargs="?", default=None, help="Caminho do JSON de dados (argumento posicional)"
    )
    parser.add_argument(
        "output_positional", nargs="?", default=None, help="Caminho do PDF de saída (argumento posicional)"
    )
    parser.add_argument("--input", "-i", dest="input_file", default=None, help="Caminho do JSON de dados")
    parser.add_argument("--output", "-o", dest="output_file", default=None, help="Caminho do PDF de saída")
    parser.add_argument(
        "--pacote",
        "-p",
