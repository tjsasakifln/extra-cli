...
def _build_oportunidades(wb: Workbook, items: list[dict], capacity_10x: float | None):
    ...
    for row_num, item in enumerate(sorted_items, start=2):
        ...
        # Col 22: Valor Esperado EV (R$) — EV = P(vitória) × margem líquida × valor estimado
        valor_esperado = None
        if isinstance(bid_sim, dict):
            valor_esperado = _safe_float(bid_sim.get("valor_esperado"))
        if valor_esperado is not None:
            row.append(_dc(22, value=valor_esperado, number_format=CURRENCY_FMT))
        else:
        ...
        # Col 24: Margem Líquida
        margem = None
        if isinstance(bid_sim, dict):
            margem_pct = _safe_float(bid_sim.get("margem_liquida_pct"))
            if margem_pct is not None:
                margem = margem_pct / 100.0
        if margem is not None:
            row.append(_dc(24, value=margem, number_format="0%"))
        else:
        ...
        row.append(_dc(26, value=cnae_label, font=cnae_font))
        ...
        if cnae_conf is not None:
            ...
            row.append(_dc(27, value=cnae_conf_display, font=conf_font, number_format="0%"))
        else:
            row.append(_dc(27, value=""))

        # Col 28: Dentro Capacidade
        if cap_label == "SIM":
        ...
        row.append(_dc(28, value=cap_label, font=cap_font))
        ...
        if _cnae_conf_raw is not None:
            ...
            rel_label, rel_font = "M\u00e9dia", st["amber_font"]
        row.append(_dc(29, value=rel_label, font=rel_font))

        # Col 30: Setor
        row.append(_dc(30, value=_sanitize(sector_name)))
