...
def measure_contact_coverage(
    *,
    target_confirmed_keys: list[str] | set[str],
    ...
    confirmed = {str(k) for k in target_confirmed_keys}
    ...
    never = confirmed - attempted
    # Intersections should be subsets of confirmed when measuring reservoir coverage
    attempted_of_confirmed = attempted & confirmed
    real_of_confirmed = real_email & confirmed
    owned_of_confirmed = company_owned & confirmed
    identity_of_confirmed = identity_safe & confirmed
    esr_of_confirmed = send_ready & confirmed

    n_conf = len(confirmed)
    n_att = len(attempted_of_confirmed)
    n_never = len(never)
    ...
    return {
        ...
        "TARGET_CONFIRMED_total": n_conf,
        "contact_discovery_attempted": n_att,
        "contact_discovery_not_attempted": n_never,
        "contact_discovery_attempt_rate": _rate(n_att, n_conf),
        "real_email_found": len(real_of_confirmed),
        "real_email_rate_of_attempted": _rate(len(real_of_confirmed), n_att),
        "company_owned_email": len(owned_of_confirmed),
        "identity_safe": len(identity_of_confirmed),
        "email_send_ready": len(esr_of_confirmed),
        "email_send_ready_of_confirmed": _rate(len(esr_of_confirmed), n_conf),
        "email_send_ready_of_attempted": _rate(len(esr_of_confirmed), n_att),
        "rejection_reasons": reasons,
        "MINIMUM_PILOT_ACCEPTANCE_SAMPLE": MINIMUM_PILOT_ACCEPTANCE_SAMPLE,
        "pilot_sample_met": len(esr_of_confirmed) >= MINIMUM_PILOT_ACCEPTANCE_SAMPLE,
        "note": (
            "EMAIL_SEND_READY reservoir is not capped at 50. "
            "50 is MINIMUM_PILOT_ACCEPTANCE_SAMPLE only."
        ),
        "closed_sum_check": {
            "confirmed_eq_attempted_plus_never": n_conf == n_att + n_never,
            "attempted_subset_of_confirmed": attempted_of_confirmed <= confirmed,
        },
    }
