"""PNCP PCA crawler — Planos de Contratacoes Anuais.

EXT-010: Crawls PCA data from PNCP ``/api/consulta/v1/pca`` and upserts
into ``pncp_raw_pca`` via the ``upsert_pncp_raw_pca`` RPC.

PCA (Planos de Contratacoes Anuais) are yearly procurement plans published
by each agency. They allow suppliers to anticipate demand 6-12 months before
the actual bid is published.

Volume estimate:
  - Nacional: ~280k items/year
  - SC: ~15k items/year

Crawl strategy:
  - Weekly (Monday 6am BRT) for all configured UFs
  - Current year by default
  - Content_hash dedup via (pca_id, data_publicacao)
"""

from __future__ import annotations

import asyncio
import hashlib
import json
import logging
from dataclasses import dataclass, field
from datetime import date, datetime
from typing import Any

import httpx

from ingestion._base.crawler import accumulate_stats, chunk_list, empty_run_stats
from ingestion.config import (
    INGESTION_PCA_ENABLED,
    INGESTION_PCA_MAX_PAGES,
    INGESTION_BATCH_DELAY_S,
    INGESTION_BATCH_SIZE_UFS,
    INGESTION_CONCURRENT_UFS,
    INGESTION_UPSERT_BATCH_SIZE,
    INGESTION_UFS,
)
from ingestion.metrics import (
    PCA_RECORDS_FETCHED,
    PCA_RECORDS_UPSERTED,
    PCA_RUNS_TOTAL,
    PCA_RUN_DURATION,
    PCA_PAGES_FETCHED,
)

logger = logging.getLogger(__name__)

PNCP_BASE_URL = "https://pncp.gov.br/api/consulta/v1"
PNCP_PCA_ENDPOINT = "/pca"

_HTTP_TIMEOUT = httpx.Timeout(30.0, connect=10.0)


# ---------------------------------------------------------------------------
# Content hash helper
# ---------------------------------------------------------------------------


def _make_content_hash(pca_id: str, data_publicacao: str | None) -> str:
    """Generate a deterministic content hash for dedup.

    PCA content_hash is computed from (pca_id, data_publicacao).
    """
    raw = f"{pca_id}|{data_publicacao or ''}"
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()


# ---------------------------------------------------------------------------
# API response normaliser
# ---------------------------------------------------------------------------


def _normalise_pca_item(raw: dict, uf: str) -> dict | None:
    """Normalise a raw PNCP PCA API response into a pncp_raw_pca row.

    Args:
        raw: A single item from the PNCP /pca API response array.
        uf: The UF being crawled (used as fallback).

    Returns:
        A dict ready for upsert, or None if the item is malformed.
    """
    try:
        pca_id = str(raw.get("identificadorPca") or raw.get("id") or "")
        if not pca_id:
            return None

        # Extract orgao info
        orgao = raw.get("orgaoEntidade") or raw.get("orgao") or {}
        orgao_razao = (
            orgao.get("razaoSocial")
            or orgao.get("nome")
            or raw.get("orgaoRazaoSocial")
            or ""
        )
        orgao_cnpj = (
            orgao.get("cnpj")
            or orgao.get("cpfCnpj")
            or raw.get("orgaoCnpj")
            or ""
        )
        orgao_cnpj_clean = "".join(c for c in orgao_cnpj if c.isdigit())

        # UF with fallback
        uf_value = (
            (orgao.get("uf") if isinstance(orgao, dict) else None)
            or raw.get("uf")
            or uf
        )

        # Category: material, servico, obra
        categoria_raw = raw.get("categoria") or raw.get("tipoItem") or ""
        categoria = str(categoria_raw).lower().strip()
        # Map to known categories
        categoria_map = {
            "material": "material",
            "servico": "servico",
            "obra": "obra",
            "materials": "material",
            "servicos": "servico",
            "obras": "obra",
        }
        categoria = categoria_map.get(categoria, categoria or "material")

        # Dates
        data_publicacao = raw.get("dataPublicacao") or raw.get("dataInclusao") or ""

        row = {
            "pca_id": pca_id,
            "orgao_razao_social": orgao_razao,
            "orgao_cnpj": orgao_cnpj_clean,
            "uf": uf_value.upper() if uf_value else uf,
            "municipio": str(
                raw.get("municipio")
                or (orgao.get("municipio") if isinstance(orgao, dict) else None)
                or ""
            ),
            "ano_exercicio": int(raw.get("anoExercicio") or raw.get("ano") or date.today().year),
            "categoria": categoria,
            "descricao_item": str(raw.get("descricaoItem") or raw.get("descricao") or raw.get("objeto") or ""),
            "quantidade_estimada": float(raw.get("quantidadeEstimada") or raw.get("quantidade") or 0),
            "unidade_medida": str(raw.get("unidadeMedida") or raw.get("unidade") or ""),
            "valor_estimado": float(raw.get("valorEstimado") or raw.get("valorTotal") or 0),
            "data_publicacao": str(data_publicacao)[:10] if data_publicacao else None,
            "content_hash": _make_content_hash(pca_id, str(data_publicacao)[:10] if data_publicacao else None),
            "source": "pncp_pca",
        }
        return row
    except (ValueError, TypeError, AttributeError) as exc:
        logger.debug("PCA normalise error (skipped): %s — raw=%s", exc, str(raw)[:200])
        return None


# ---------------------------------------------------------------------------
# Crawler class
# ---------------------------------------------------------------------------


@dataclass
class PcaCrawlResult:
    """Result from a PCA crawl run per UF."""

    fetched: int = 0
    inserted: int = 0
    updated: int = 0
    unchanged: int = 0
    pages: int = 0
    errors: int = 0
    uf: str = ""
    extra: dict[str, Any] = field(default_factory=dict)


class PncpPcaCrawler:
    """Crawler for Planos de Contratacoes Anuais (PCA) from PNCP.

    API: GET /api/consulta/v1/pca
    Parameters: uf, anoExercicio, pagina, tamanhoPagina
    """

    SOURCE = "pncp_pca"
    ENDPOINT = PNCP_PCA_ENDPOINT

    def __init__(self, ufs: list[str] | None = None) -> None:
        self.ufs = ufs or INGESTION_UFS
        self._client: httpx.AsyncClient | None = None

    async def __aenter__(self) -> "PncpPcaCrawler":
        self._client = httpx.AsyncClient(
            base_url=PNCP_BASE_URL,
            timeout=_HTTP_TIMEOUT,
            headers={
                "User-Agent": "SmartLic/1.0 (procurement-search; contato@smartlic.tech)",
                "Accept": "application/json",
            },
            follow_redirects=True,
        )
        return self

    async def __aexit__(self, *args: Any) -> None:
        if self._client:
            await self._client.aclose()

    async def _fetch_page_raw(
        self,
        uf: str,
        ano_exercicio: int,
        page: int = 1,
    ) -> tuple[list[dict], dict]:
        """Fetch a single page of PCA records.

        Returns:
            Tuple of (items_list, full_response_dict) for pagination handling.
        """
        if self._client is None:
            raise RuntimeError("Client not initialised. Use async context manager.")

        params = {
            "uf": uf,
            "anoExercicio": ano_exercicio,
            "pagina": page,
            "tamanhoPagina": 50,
        }

        response = await self._client.get(self.ENDPOINT, params=params)

        if response.status_code == 204:
            return [], {}

        if response.status_code != 200:
            logger.warning(
                "PCA: uf=%s ano=%d page=%d HTTP %d — %s",
                uf, ano_exercicio, page, response.status_code, response.text[:200],
            )
            response.raise_for_status()

        data = response.json()
        items: list[dict] = data.get("data") or []
        return items, data

    def _has_more(self, response_data: dict) -> bool:
        """Check if more pages are available."""
        return bool(
            response_data.get("temProximaPagina")
            or int(response_data.get("paginasRestantes", 0)) > 0
        )

    async def crawl_uf(
        self,
        uf: str,
        ano_exercicio: int,
        max_pages: int = INGESTION_PCA_MAX_PAGES,
    ) -> PcaCrawlResult:
        """Crawl PCA for a single UF and year.

        Iterates pages, normalises items, returns collected rows without
        upserting (upsert happens at the batch level).
        """
        result = PcaCrawlResult(uf=uf)
        all_rows: list[dict] = []
        page = 1
        has_more = True

        while page <= max_pages and has_more:
            try:
                items, resp_data = await self._fetch_page_raw(uf, ano_exercicio, page)

                if not items:
                    break

                PCA_PAGES_FETCHED.labels(uf=uf).inc()
                result.pages += 1

                for item in items:
                    row = _normalise_pca_item(item, uf)
                    if row is not None:
                        all_rows.append(row)

                PCA_RECORDS_FETCHED.labels(uf=uf).inc(len(items))

                has_more = self._has_more(resp_data)
                page += 1

            except (httpx.HTTPError, asyncio.TimeoutError) as exc:
                logger.warning(
                    "PCA: uf=%s ano=%d page=%d error — %s: %s",
                    uf, ano_exercicio, page, type(exc).__name__, exc,
                )
                result.errors += 1
                break

        result.fetched = len(all_rows)
        result.extra["rows"] = all_rows
        return result

    async def crawl_all_ufs(
        self,
        ano_exercicio: int | None = None,
    ) -> dict[str, Any]:
        """Crawl PCA for all configured UFs in parallel batches.

        Args:
            ano_exercicio: Year to crawl (default: current year).

        Returns:
            dict with aggregated stats.
        """
        ano = ano_exercicio or date.today().year
        totals = empty_run_stats()
        semaphore = asyncio.Semaphore(INGESTION_CONCURRENT_UFS)

        async def _crawl_single_uf(uf: str) -> dict[str, Any]:
            async with semaphore:
                result = await self.crawl_uf(uf, ano)

            rows = result.extra.get("rows", [])
            if rows:
                counts = await _upsert_pca_batch(rows)
                result.inserted = counts.get("inserted", 0)
                result.updated = counts.get("updated", 0)
                result.unchanged = counts.get("unchanged", 0)

                PCA_RECORDS_UPSERTED.labels(
                    uf=uf, action="inserted",
                ).inc(result.inserted)
                PCA_RECORDS_UPSERTED.labels(
                    uf=uf, action="updated",
                ).inc(result.updated)

            uf_stats = {
                "fetched": result.fetched,
                "inserted": result.inserted,
                "updated": result.updated,
                "unchanged": result.unchanged,
                "ufs_crawled": result.pages,
                "ufs_failed": result.errors,
            }
            accumulate_stats(totals, uf_stats)
            return uf_stats

        uf_batches = chunk_list(self.ufs, INGESTION_BATCH_SIZE_UFS)

        for batch_idx, uf_batch in enumerate(uf_batches):
            if batch_idx > 0:
                await asyncio.sleep(INGESTION_BATCH_DELAY_S)

            tasks = [_crawl_single_uf(uf) for uf in uf_batch]
            await asyncio.gather(*tasks, return_exceptions=False)

        return {
            "fetched": totals["fetched"],
            "inserted": totals["inserted"],
            "updated": totals["updated"],
            "unchanged": totals["unchanged"],
            "pages": totals["ufs_crawled"],
            "errors": totals["ufs_failed"],
        }


# ---------------------------------------------------------------------------
# Upsert helper
# ---------------------------------------------------------------------------


async def _upsert_pca_batch(rows: list[dict]) -> dict[str, int]:
    """Upsert PCA rows into pncp_raw_pca via Supabase RPC."""
    if not rows:
        return {"inserted": 0, "updated": 0, "unchanged": 0}

    from supabase_client import get_supabase, sb_execute

    totals = {"inserted": 0, "updated": 0, "unchanged": 0}
    batches = [rows[i:i + INGESTION_UPSERT_BATCH_SIZE] for i in range(0, len(rows), INGESTION_UPSERT_BATCH_SIZE)]
    supabase = get_supabase()

    for batch_idx, batch in enumerate(batches):
        try:
            payload = json.loads(json.dumps(batch, default=str, ensure_ascii=False))

            result = await sb_execute(
                supabase.rpc("upsert_pncp_raw_pca", {"p_records": payload}),
                category="rpc",
            )

            counts = _extract_rpc_counts(result)
            totals["inserted"] += counts.get("inserted", 0)
            totals["updated"] += counts.get("updated", 0)
            totals["unchanged"] += counts.get("unchanged", 0)

        except Exception as exc:
            logger.error(
                "PCA upsert batch %d failed: %s: %s — continuing",
                batch_idx + 1, type(exc).__name__, exc,
            )
            continue

    return totals


def _extract_rpc_counts(result: Any) -> dict[str, int]:
    """Extract inserted/updated/unchanged from RPC result."""
    try:
        data = result.data
        if isinstance(data, list) and data:
            row = data[0]
            return {
                "inserted": int(row.get("inserted", 0)),
                "updated": int(row.get("updated", 0)),
                "unchanged": int(row.get("unchanged", 0)),
            }
        return {"inserted": 0, "updated": 0, "unchanged": 0}
    except Exception:
        return {"inserted": 0, "updated": 0, "unchanged": 0}


# ---------------------------------------------------------------------------
# Module-level entry points (for ARQ scheduler)
# ---------------------------------------------------------------------------


async def crawl_pca_weekly(
    *,
    ano_exercicio: int | None = None,
) -> dict[str, Any]:
    """Weekly PCA crawl across all configured UFs.

    Crawls PCA data for the given year (default: current year).

    Returns:
        Aggregated statistics dict.
    """
    if not INGESTION_PCA_ENABLED:
        logger.info("[PCA] Skipped — INGESTION_PCA_ENABLED=false")
        PCA_RUNS_TOTAL.labels(status="skipped").inc()
        return {"status": "skipped", "reason": "INGESTION_PCA_ENABLED=false"}

    ano = ano_exercicio or date.today().year
    start = datetime.utcnow()
    logger.info("[PCA] Starting weekly crawl — ano=%d ufs=%d", ano, len(INGESTION_UFS))

    try:
        async with PncpPcaCrawler() as crawler:
            result = await crawler.crawl_all_ufs(ano_exercicio=ano)
    except Exception as exc:
        duration = (datetime.utcnow() - start).total_seconds()
        logger.error("[PCA] Crawl failed: %s: %s", type(exc).__name__, exc, exc_info=True)
        PCA_RUNS_TOTAL.labels(status="failed").inc()
        return {"status": "failed", "error": str(exc), "duration_s": round(duration, 1)}

    duration = (datetime.utcnow() - start).total_seconds()
    PCA_RUNS_TOTAL.labels(status="completed").inc()
    PCA_RUN_DURATION.observe(duration)

    logger.info(
        "[PCA] Completed in %.1fs — fetched=%d inserted=%d updated=%d unchanged=%d errors=%d",
        duration,
        result.get("fetched", 0),
        result.get("inserted", 0),
        result.get("updated", 0),
        result.get("unchanged", 0),
        result.get("errors", 0),
    )

    return {
        "status": "completed",
        **result,
        "duration_s": round(duration, 1),
    }
