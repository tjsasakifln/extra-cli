...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    # PNCP live status is acquisition telemetry, never a commercial gate. The
    # outbound plane is authorised by the persisted datalake, so a STALE/UNKNOWN
    # or unreachable source degrades observability and nothing else. The status
    # is reported verbatim: a missing or failed probe is never fabricated FRESH.
    ...
    if dsn:
        try:
            ...
            if contract.get("status") == "FRESH":
                observed = datetime.fromisoformat(str(contract["as_of"]).replace("Z", "+00:00"))
                ...
                expires_at = observed + timedelta(hours=max(0.0, target_hours - lag_hours))
                authoritative_source_freshness["expires_at"] = expires_at.isoformat().replace("+00:00", "Z")
        except Exception as exc:  # noqa: BLE001 - telemetry must not block durable authority
            authoritative_source_freshness = {
