"""IBGE population bands for municipal prioritization (contextual only)."""
...
_DEFAULT_POP = _ROOT / "config/municipio_population.yaml"

# Minimal SC IBGE code → name map for common municipalities (extendable).
# Codes align with IBGE; names used for display when contracts only have orgao_nome.
_SC_IBGE_NAMES: dict[str, str] = {
...
_SUPPLEMENT = _ROOT / "config/commercial/sc_municipality_population_censo2022_supplement.yaml"
...
def lookup_municipality_name(ibge_code: str | None) -> str | None:
    if not ibge_code:
        return None
    return _SC_IBGE_NAMES.get(str(ibge_code))
...
def _normalize_agency_name(name: str) -> str:
    ...
    for prefix in (
        ...
        if n.startswith(prefix):
            n = n[len(prefix) :]
            break
    n = re.sub(r"\s*[-/]?\s*SC\s*$", "", n).strip(" -")
    return n.strip()


def match_population_by_name(orgao_nome: str | None, pop_map: dict[str, int] | None = None) -> dict[str, Any]:
    """Best-effort match of PREFEITURA/MUNICÍPIO DE X → IBGE pop for SC."""
    raw = orgao_nome or ""
    name = _fold(raw)
    core = _normalize_agency_name(raw)
    pops = pop_map if pop_map is not None else load_population_map()
    name_to_code = {_fold(v): k for k, v in _SC_IBGE_NAMES.items()}

    # exact core match first
    if core in name_to_code:
        code = name_to_code[core]
        pop = pops.get(code)
        return {
            "ibge_code": code,
            "municipio": _SC_IBGE_NAMES[code],
            "population": pop,
            "population_band": population_band(pop),
            "match": "core_exact",
        }

    # longest substring match (avoid short false positives)
    best: tuple[int, str, str] | None = None  # len, mun_name, code
    for mun_name, code in name_to_code.items():
        if mun_name in name or mun_name in core or (len(core) >= 5 and core in mun_name):
            if best is None or len(mun_name) > best[0]:
                best = (len(mun_name), mun_name, code)
