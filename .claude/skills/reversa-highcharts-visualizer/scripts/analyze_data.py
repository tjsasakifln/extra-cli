...
def suggest_charts(categories: list, series_analysis: list, n_series: int) -> list:
    ...
    if temporal:
        suggestions.append(
            {"type": "line", "score": 95, "reason": "Dados temporais — ideal para mostrar tendência ao longo do tempo"}
        )
        suggestions.append({"type": "area", "score": 85, "reason": "Dados temporais — área enfatiza volume/magnitude"})
    ...
    if n_series == 1 and n_categories <= 8 and all_positive:
        suggestions.append(
            {"type": "pie", "score": 80, "reason": "Uma série com poucas categorias — mostra proporção/composição"}
        )

    # Duas séries numéricas → scatter
    if n_series >= 2 and all(s.get("type") == "numeric" for s in series_analysis):
        suggestions.append(
            {"type": "scatter", "score": 70, "reason": "Múltiplas séries numéricas — mostra correlação entre variáveis"}
        )

    # Muitos dados → considerar heatmap
    if n_categories > 20 and n_series > 5:
        suggestions.append(
            {"type": "heatmap", "score": 75, "reason": "Muitas categorias × séries — heatmap revela padrões matriciais"}
        )

    # KPI único → gauge
    if n_series == 1 and n_categories == 1:
        suggestions.append(
            {"type": "solidgauge", "score": 85, "reason": "Valor único — ideal para KPI/indicador de progresso"}
        )
...
def main():
    ...
    if args.format == "json":
        ...
        for s in series_analysis:
            print(
                f"  • {s['name']}: min={s.get('min')}, max={s.get('max')}, "
