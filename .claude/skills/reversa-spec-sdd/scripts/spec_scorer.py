...
def has_contradictions_signal(text: str) -> bool:
    ...
    return bool(re.search(r"RF-\d+.*?\b(mas|porém|entretanto|however|but)\b", text, re.IGNORECASE | re.DOTALL))
...
def score_edge_cases(text: str) -> DimensionScore:
    ...
    covers_external = bool(
        re.search(r"(timeout|indisponível|fora do ar|falha|erro\s+\d{3}|retry|fallback)", ec_section, re.IGNORECASE)
    )
    if covers_external:
        score += 2
...
def classify(score: float) -> str:
    if score >= 90:
        return "⭐ Excelente — Pronta para implementação"
    if score >= 80:
        return "✅ Boa — Pronta com ajustes menores"
    if score >= 65:
        return "⚠️  Adequada — Implementável com riscos"
    if score >= 50:
        return "🔶 Incompleta — Revisar antes de implementar"
...
def build_report(spec_path: str) -> SpecReport:
    ...
    critical_gaps = [issue for d in dims for issue in d.issues if issue.startswith("❌")]
    suggestions = [issue for d in dims for issue in d.issues if issue.startswith("⚠️")]
...
def main():
    parser = argparse.ArgumentParser(description="Avalia a qualidade de uma spec de feature (0–100)")
