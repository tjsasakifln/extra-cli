# CONFENGE contact enrichment — human review

Generated: 2026-08-08T20:26:37Z

Status: **HUMAN_REVIEW_PENDING** (machine may be PASS; human must review)

Each case lists company, CNPJ, contact, ownership, confidence, reason, source, domain.

## Accepted / enrollable (20)

### 1. PAVSANTOS CONSTRUTORA LTDA (`03575041000102`)

- **contato:** `pavsantos@pavsantos.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.98
- **reason:** domain_match_company_owned
- **source_type:** site
- **source_url:** https://pavsantos.com.br/
- **domínio oficial:** pavsantos.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 2. ELEVADORES ATLAS SCHINDLER LTDA. (`00028986000108`)

- **contato:** `+5511933328527`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.65
- **reason:** phone_on_official_company_source_single_holder
- **source_type:** site
- **source_url:** https://elevadoresatlasschindler.com/
- **domínio oficial:** elevadoresatlasschindler.com
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** OBSERVED
- **enrollable:** True

### 3. SCAVE SERVICOS DE ENGENHARIA E LOCACAO LTDA (`01514128000136`)

- **contato:** `+558134263243`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.65
- **reason:** phone_on_official_company_source_single_holder
- **source_type:** contact_page
- **source_url:** https://scave.com.br/contato
- **domínio oficial:** scave.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** OBSERVED
- **enrollable:** True

### 4. AEGEA SANEAMENTO E PARTICIPACOES S.A. (`08827501000158`)

- **contato:** `relacionamento.ro@aegea.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.98
- **reason:** domain_match_company_owned
- **source_type:** contact_page
- **source_url:** https://aegea.com.br/contato
- **domínio oficial:** aegea.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 5. ENCOPAV ENGENHARIA LTDA (`00061493000170`)

- **contato:** `encopav@encopav.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.98
- **reason:** domain_match_company_owned
- **source_type:** contact_page
- **source_url:** https://encopav.com.br/contato
- **domínio oficial:** encopav.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 6. METRA CONSTRUCOES LTDA (`34348652000133`)

- **contato:** `atendimento@metra.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 1.0
- **reason:** domain_match_company_owned
- **source_type:** contact_page
- **source_url:** https://metra.com.br/fale-conosco
- **domínio oficial:** metra.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 7. VENTO SUL ENGENHARIA LTDA (`03509843000106`)

- **contato:** `ventosul@ventosul.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.98
- **reason:** domain_match_company_owned
- **source_type:** contact_page
- **source_url:** https://ventosul.com.br/contato
- **domínio oficial:** ventosul.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 8. MOVE INFRAESTRUTURA LTDA. (`05296490000139`)

- **contato:** `vagas@moveinfraestrutura.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.98
- **reason:** domain_match_company_owned
- **source_type:** contact_page
- **source_url:** https://moveinfraestrutura.com.br/contato
- **domínio oficial:** moveinfraestrutura.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 9. CONCRELAJE INDUSTRIA DE PRE-FABRICADOS DE CONCRETO LTDA (`01557107000106`)

- **contato:** `concrelaje@concrelaje.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.88
- **reason:** domain_match_company_owned
- **source_type:** site
- **source_url:** https://concrelaje.com.br/
- **domínio oficial:** concrelaje.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 10. TRACADO CONSTRUCOES E SERVICOS LTDA (`00472805000138`)

- **contato:** `comercial@tracado.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 1.0
- **reason:** domain_match_company_owned
- **source_type:** contact_page
- **source_url:** https://tracado.com.br/fale-conosco
- **domínio oficial:** tracado.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 11. CONSTRUVERDE CONSTRUCOES E SERVICOS LTDA (`20360878000145`)

- **contato:** `+551138129000`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.65
- **reason:** phone_on_official_company_source_single_holder
- **source_type:** contact_page
- **source_url:** https://construverde.com.br/contato
- **domínio oficial:** construverde.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** OBSERVED
- **enrollable:** True

### 12. CONSTRUTORA ANCAR LTDA (`00758756000102`)

- **contato:** `+551120403635`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.65
- **reason:** phone_on_official_company_source_single_holder
- **source_type:** site
- **source_url:** https://ancar.com.br/
- **domínio oficial:** ancar.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** OBSERVED
- **enrollable:** True

### 13. PAVCON CONSTRUTORA LTDA (`15747692000103`)

- **contato:** `contato@pavcon.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 1.0
- **reason:** domain_match_company_owned
- **source_type:** contact_page
- **source_url:** https://pavcon.com.br/contato
- **domínio oficial:** pavcon.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 14. EXITO SERVICOS ESPECIALIZADOS LTDA (`32250463000106`)

- **contato:** `+5561981548106`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.65
- **reason:** phone_on_official_company_source_single_holder
- **source_type:** contact_page
- **source_url:** https://exito.com.br/contato
- **domínio oficial:** exito.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** OBSERVED
- **enrollable:** True

### 15. QUALIDADE MINERACAO LTDA (`00820854000114`)

- **contato:** `contato@qualidademineracao.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 1.0
- **reason:** domain_match_company_owned
- **source_type:** contact_page
- **source_url:** https://qualidademineracao.com.br/contato
- **domínio oficial:** qualidademineracao.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 16. EXCEED SERVICOS DIGITAIS LTDA. (`07852038000131`)

- **contato:** `comercial@exceed.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 1.0
- **reason:** domain_match_company_owned
- **source_type:** site
- **source_url:** https://exceed.com.br/
- **domínio oficial:** exceed.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 17. COMARO ENGENHARIA LTDA (`00794165000182`)

- **contato:** `comercial@comaro.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 1.0
- **reason:** domain_match_company_owned
- **source_type:** contact_page
- **source_url:** https://comaro.com.br/contato
- **domínio oficial:** comaro.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 18. SANEAMENTO DE GOIAS S/A (`01616929000102`)

- **contato:** `+551135790891`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 0.65
- **reason:** phone_on_official_company_source_single_holder
- **source_type:** site
- **source_url:** https://saneago.com.br/
- **domínio oficial:** saneago.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** OBSERVED
- **enrollable:** True

### 19. SIMENG ENGENHARIA LTDA (`60025322000117`)

- **contato:** `contato@simeng.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 1.0
- **reason:** domain_match_company_owned
- **source_type:** contact_page
- **source_url:** https://simeng.com.br/contato
- **domínio oficial:** simeng.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

### 20. MIDT ENGENHARIA LTDA (`40301626000152`)

- **contato:** `contato@midt.com.br`
- **ownership_status:** COMPANY_OWNED
- **confidence:** 1.0
- **reason:** domain_match_company_owned
- **source_type:** site
- **source_url:** https://midt.com.br/
- **domínio oficial:** midt.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** True

## Rejected (20)

### 1. LMA ENGENHARIA LTDA (`51694102000195`)

- **contato:** `+557496969728`
- **ownership_status:** SHARED_EXTERNAL_CONTACT
- **confidence:** 0.15
- **reason:** Phone shared by 4 unrelated CNPJs
- **source_type:** —
- **source_url:** —
- **domínio oficial:** lma.com.br
- **reuse signal (associated companies):** 5
- **third-party signals:** —
- **verification_status:** OBSERVED
- **enrollable:** False

### 2. CONSTRUTORA LUIZ COSTA LTDA (`00779059000120`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** clc.com
- **reuse signal (associated companies):** 6
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 3. TMF CONSTRUCOES, INCORPORACAO E SERVICOS LTDA (`36909349000198`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** tmf.com.br
- **reuse signal (associated companies):** 7
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 4. SUPERA ENGENHARIA LTDA (`38756140000167`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** supera.com.br
- **reuse signal (associated companies):** 8
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 5. J G A BODANESE ENGENHARIA LTDA (`41659936000106`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** bodanese.com.br
- **reuse signal (associated companies):** 9
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 6. COSAMPA CONSTRUCOES LTDA (`03006548000137`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** cosampa.com.br
- **reuse signal (associated companies):** 10
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 7. GAIA RODOVIAS LTDA. (`03257777000124`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** gaiarodovias.com.br
- **reuse signal (associated companies):** 11
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 8. NORTE CONSTRUCOES LTDA (`24581449000159`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** norte.com
- **reuse signal (associated companies):** 12
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 9. ABTEC ENGENHARIA LTDA (`12754237000147`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** abtec.com.br
- **reuse signal (associated companies):** 13
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 10. JATOBETON ENGENHARIA LTDA (`00507949000182`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** jatobeton.com.br
- **reuse signal (associated companies):** 14
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 11. PAVIPLAN PAVIMENTACAO LTDA (`03620927000112`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** paviplan.com.br
- **reuse signal (associated companies):** 15
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 12. BETUNEL INDUSTRIA E COMERCIO S/A (`60546801000260`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** betunel.com.br
- **reuse signal (associated companies):** 16
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 13. BANCO DO BRASIL SA (`00000000037788`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** formosago.com.br
- **reuse signal (associated companies):** 17
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 14. PARAIBA TURISMO LTDA (`00455771000173`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** paraiba.com.br
- **reuse signal (associated companies):** 18
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 15. TRACADO CONSTRUCOES E SERVICOS LTDA (`00472805000308`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** tracado.com.br
- **reuse signal (associated companies):** 19
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 16. LPK LTDA (`00535560000140`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** lpk.com
- **reuse signal (associated companies):** 20
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 17. VISOMES COMERCIAL METROLOGICA LTDA (`00567892000107`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** visomes.com.br
- **reuse signal (associated companies):** 21
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 18. ALVARO MARQUES REIS SONORIZACOES LTDA (`00636230000141`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** am.com
- **reuse signal (associated companies):** 22
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 19. VERBENNA FARMACIA DE MANIPULACAO LTDA (`00787540000167`)

- **contato:** `hello@yourdomain.com`
- **ownership_status:** SHARED_EXTERNAL_CONTACT
- **confidence:** 0.175
- **reason:** Channel shared by 19 unrelated CNPJs without known economic group or matriz/filial relation
- **source_type:** —
- **source_url:** —
- **domínio oficial:** verbenna.com
- **reuse signal (associated companies):** 20
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

### 20. J D DE OLIVEIRA & I L CRUAIA LTDA (`00978237000141`)

- **contato:** `+557496969728`
- **ownership_status:** THIRD_PARTY_SERVICE_PROVIDER
- **confidence:** 0.1155
- **reason:** Known third-party registry hit: None (OTHER)
- **source_type:** —
- **source_url:** —
- **domínio oficial:** oliveira.com.br
- **reuse signal (associated companies):** 23
- **third-party signals:** OTHER
- **verification_status:** OBSERVED
- **enrollable:** False

## Unresolved / review required (20)

### 1. AMF ENGENHARIA E SERVICOS LTDA (`00638562000165`)

- **contato:** `amf@amf.com.br`
- **ownership_status:** UNRESOLVED
- **confidence:** 0.1201
- **reason:** weak_or_conflicting_ownership_signals
- **source_type:** site
- **source_url:** https://amf.com.br/
- **domínio oficial:** amf.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** VERIFIED
- **enrollable:** False

### 2. SUPERAGE ENGENHARIA S/A (`20558479000193`)

- **contato:** `+553135418664`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** superage.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 3. TETT'S ENGENHARIA LTDA (`47815614000195`)

- **contato:** `+556282596100`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** tett.com
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 4. CONSTRUTORA E TRANSPORTADORA IDEAL LTDA (`08815415000125`)

- **contato:** `+5511981786900`
- **ownership_status:** UNRESOLVED
- **confidence:** 0.2275
- **reason:** phone_source_host_not_company_aligned
- **source_type:** site
- **source_url:** https://transportadora.com.br/
- **domínio oficial:** transportadora.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** OBSERVED
- **enrollable:** False

### 5. R3 PRESTACAO DE SERVICOS LTDA (`15371451000102`)

- **contato:** `+554999980884`
- **ownership_status:** UNRESOLVED
- **confidence:** 0.175
- **reason:** phone_source_host_not_company_aligned
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** r3.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 6. CONSORCIO COQUEIROS ARACAJU (`65457635000168`)

- **contato:** `+551121246140`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** consorcio.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 7. VILLARES CONSTRUTORA E METALURGICA LTDA (`05863476000170`)

- **contato:** `+554436487027`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** villares.com
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 8. PELT PROJETOS E CONSTRUCOES LTDA (`15583839000177`)

- **contato:** `+552166512112`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** pelt.com
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 9. SIX PAVIMENTACAO LTDA (`03752550000155`)

- **contato:** `+5541992768954`
- **ownership_status:** UNRESOLVED
- **confidence:** 0.2275
- **reason:** phone_source_host_not_company_aligned
- **source_type:** site
- **source_url:** https://six.com.br/
- **domínio oficial:** six.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** OBSERVED
- **enrollable:** False

### 10. SIMEMP SERVICOS TECNICOS E OBRAS LTDA (`09237296000133`)

- **contato:** `+556135464530`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** simemp.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 11. MATT CONSTRUTORA LTDA (`00220982000127`)

- **contato:** `+551786097998`
- **ownership_status:** UNRESOLVED
- **confidence:** 0.2275
- **reason:** phone_source_host_not_company_aligned
- **source_type:** site
- **source_url:** https://mpx.com.br/
- **domínio oficial:** mpx.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** OBSERVED
- **enrollable:** False

### 12. ARQUIUM - CONSTRUCOES E RESTAURO LTDA. (`06939632000100`)

- **contato:** `+555132467701`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** arquium.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 13. WFL COMERCIAL E PRESTADORA LTDA (`17077946000159`)

- **contato:** `+556298376353`
- **ownership_status:** UNRESOLVED
- **confidence:** 0.175
- **reason:** phone_source_host_not_company_aligned
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** wfl.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 14. COFRANZA CONSTRUTORA LTDA (`03396992000106`)

- **contato:** `+552227252503`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** francisco.com
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 15. MANUTEC MONTAGEM E EMPREENDIMENTOS LTDA (`18546232000105`)

- **contato:** `+552196912473`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** manutec.com
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 16. PROJECON - PROJETOS E CONSTRUCOES LTDA (`07765850000120`)

- **contato:** `+558132248725`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** projecon.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 17. E M T CONSTRUTORA LTDA (`05036194000107`)

- **contato:** `+556833221020`
- **ownership_status:** UNRESOLVED
- **confidence:** 0.175
- **reason:** phone_source_host_not_company_aligned
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** —
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 18. MULTICON ENGENHARIA LTDA (`00242092000116`)

- **contato:** `+551132562333`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** multicon.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

### 19. I N MARQUES (`24940072000187`)

- **contato:** `—`
- **ownership_status:** —
- **confidence:** None
- **reason:** no_public_business_contact_found
- **source_type:** —
- **source_url:** —
- **domínio oficial:** nunes.com.br
- **reuse signal (associated companies):** None
- **third-party signals:** —
- **verification_status:** —
- **enrollable:** None

### 20. CONSTRUTORA CONSTRUCERTO LTDA (`20323692000116`)

- **contato:** `+551734632833`
- **ownership_status:** LIKELY_COMPANY_OWNED
- **confidence:** 0.5
- **reason:** consistent_but_not_definitive_ownership_signals
- **source_type:** registry
- **source_url:** https://brasilapi.com.br/api/cnpj/v1/
- **domínio oficial:** construcerto.com.br
- **reuse signal (associated companies):** 1
- **third-party signals:** —
- **verification_status:** REVIEW_REQUIRED
- **enrollable:** False

## Decision

After human review, record pass/fail outside this auto-generated package.
Do not treat this file as HUMAN_REVIEW_PASS.
